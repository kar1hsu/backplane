...
Backplane 是一个可以直接用于业务开发的 Go 模块化后台管理框架。后端基于 Gin、GORM、Casbin 和 JWT，管理端使用 Vue 3，并通过 Go `embed` 打包进同一个二进制文件。

它已经准备好后台项目最常见、也最容易重复造轮子的部分：用户与角色、按钮级 RBAC、权限策略自动同步、运行时系统配置、操作审计、任务队列、文件上传和 Docker 部署。你可以保留这些基础能力，把精力放在自己的业务模块上。

> 管理端提供两套实现：`master` 使用 Element Plus；[`naive-ui`](https://github.com/kar1hsu/backplane/tree/naive-ui) 分支使用 Naive UI。
...
| 登录 | 控制台 |
| --- | --- |
| ![Element Plus 登录页](docs/images/login-element-plus.png) | ![Element Plus 控制台](docs/images/admin-element-plus.png) |

<details>
<summary>查看 Element Plus 核心功能页面</summary>

| 用户管理 | 角色管理 |
| --- | --- |
| ![Element Plus 用户管理](docs/images/users-element-plus.png) | ![Element Plus 角色管理](docs/images/roles-element-plus.png) |

| 菜单与按钮权限 | 系统配置 |
| --- | --- |
| ![Element Plus 菜单管理](docs/images/menus-element-plus.png) | ![Element Plus 系统配置](docs/images/settings-element-plus.png) |

![Element Plus 操作日志](docs/images/operation-logs-element-plus.png)

</details>

### naive-ui · Naive UI

Naive UI 重构版位于 [`naive-ui`](https://github.com/kar1hsu/backplane/tree/naive-ui) 分支：
...
<details>
<summary>查看 Naive UI 核心功能页面</summary>
...
感谢 [Gin](https://gin-gonic.com/)、[GORM](https://gorm.io/)、[Casbin](https://casbin.org/)、[Vue](https://vuejs.org/)、[Element Plus](https://element-plus.org/) 以及 [Naive UI](https://www.naiveui.com/) 等开源项目。`naive-ui` 分支的界面重构建立在 Naive UI 之上。
